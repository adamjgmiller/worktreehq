...
#[cfg(target_os = "windows")]
fn open_terminal(path: &str) -> AppResult<()> {
    // Prefer Windows Terminal; fall back to cmd.exe if wt.exe isn't on PATH.
    match Command::new("wt.exe").args(["-d", path]).spawn() {
        Ok(_) => Ok(()),
...
fn open_terminal(path: &str) -> AppResult<()> {
    ...
    // Last resort: xterm has no start-dir flag, so cd inside the shell.
    match Command::new("xterm")
        .args(["-e", &format!("cd '{}' && exec $SHELL", path)])
        .spawn()
    {
        Ok(_) => Ok(()),
