...
- App config: `~/.config/worktreehq/config.toml` (TOML; fields: `auth_method`, `github_token`, `github_token_explicitly_set`, `refresh_interval_ms`, `fetch_interval_ms`, `last_repo_path`). `auth_method` persists the user's chosen auth strategy (`"gh-cli"`, `"pat"`, or `"none"`); when absent (upgrade from an older version) the frontend auto-detects. `github_token` is a legacy migration field — new installs store PATs in the OS keychain via `keychain.rs`; on first launch of a new version the frontend migrates any plaintext token to keychain and clears this field.
