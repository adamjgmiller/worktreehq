...
- Destructive UI dialogs (`ConfirmDeleteDialog`, `RemoveWorktreeDialog`, `ForceDeleteRejectedDialog`) follow a shared shape: Escape closes, backdrop click closes, focus lands on Cancel. Typed "delete" confirmation is tiered by blast radius. Click-to-confirm: local-only branch deletes that use `git -d` (which git itself refuses for unmerged branches), clean-worktree removals without any branch cleanup, and force-deletes (`git -D`) of branches the app has classified as `merged-normally`, `squash-merged`, `direct-merged`, or `empty` — the merge detector is the safety check in those cases, with the known residual risk that the detector can misclassify (e.g. a branch name reused after a prior PR was merged, or a stale ratcheted status after `main` is rewritten). Type "delete": anything that touches a remote, force-removes a dirty worktree, discards uncommitted state, or force-deletes a local branch whose merge status is `unmerged`/`stale`/unknown. New destructive flows should match — gate typing on actual irreversibility so users don't learn to type "delete" reflexively.
