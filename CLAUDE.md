...
- Destructive UI dialogs (`ConfirmDeleteDialog`, `RemoveWorktreeDialog`, `ForceDeleteSquashDialog`) follow a shared shape: Escape closes, backdrop click closes, focus lands on Cancel. Typed "delete" confirmation is tiered by blast radius: local-only branch deletes (which use `git -d`, safe against unmerged loss) and clean-worktree removals without any branch cleanup are click-to-confirm; anything that touches a remote, force-deletes a local branch, force-removes a dirty worktree, or discards uncommitted state requires typing "delete". New destructive flows should match — gate typing on actual irreversibility so users don't learn to type "delete" reflexively.
