...
- Destructive UI dialogs (`ConfirmDeleteDialog`, `RemoveWorktreeDialog`, `ForceDeleteRejectedDialog`) follow a shared shape: Escape closes, backdrop click closes, focus lands on Cancel. Typed "delete" confirmation gates the **initial authorization** of a destructive action and is tiered by blast radius. Click-to-confirm: local-only branch deletes that use `git -d` (which git itself refuses for unmerged branches), clean-worktree removals without any branch cleanup, and force-deletes (`git -D`) of branches the app has classified as `merged-normally`, `squash-merged`, `direct-merged`, or `empty` — the merge detector is the safety check in those cases, with the known residual risk that the detector can misclassify (e.g. a branch name reused after a prior PR was merged, or a stale ratcheted status after `main` is rewritten). Type "delete": anything that **newly authorizes** a remote-touching action, force-removes a dirty worktree, discards uncommitted state, or force-deletes a local branch whose merge status is `unmerged`/`stale`/unknown. New destructive flows should match — gate typing on actual irreversibility so users don't learn to type "delete" reflexively.
- **Continuation confirmation dialogs** (e.g. `ForceDeleteRejectedDialog` opening after `git -d` refusals from an already-typed-authorized `ConfirmDeleteDialog`) tier on the *added* risk of the continuation step, not on the original authorization. The initial typed confirm in `ConfirmDeleteDialog` covers remote deletion for the whole flow; the continuation dialog's click-vs-type decision is about whether the force-delete itself is detector-vouched (click-to-confirm for a pure `squash-merged` cohort, where the PR-tag + merge-commit-equality check is an independent safety signal) or a user-vouched judgment call (typed confirm for `unmerged`/`stale` or detector-vs-git-disagreement `other` cohorts, and for any mixed cohort). Don't re-gate remote-touching modes in continuation dialogs on top of the initial typed confirm — the user already authorized; the second dialog's job is the force-delete risk, not re-authorization.
